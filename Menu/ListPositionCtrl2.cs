...
public class ListPositionCtrl2 : MonoBehaviour
{
    ...
    [Range(-1.0f, 1.0f)]
    public float listCurvature = 0.0f;
    ...
    [Range(-1.0f, 1.0f)]
    public float positionAdjust = -0.0f;
    // Set the scale ratio of the center listBox.
    public float centerBoxScaleRatio = 0.0f;
    /*===============================*/
    ...
    public void OnScroll(bool arg)
    {
        if (arg == true)
        {
            bool longtextbool = longtext.longText_active;


            if (longtextbool == false)
            {
                _scrollHandler(_test.lol);
	 ...
	 * to the center.
	 */
    public listbox2 GetCenteredBox()
    {
        float minPosition = Mathf.Infinity;
        float position;
        listbox2 candicateBox = null;
